...
const CommunityEvents: React.FC<CommunityEventsProps> = ({ communityId }) => {
  ...
  const implementSuggestion = async (suggestionId: string) => {
    try {
      ...
      setError(err.response?.data?.message || 'Failed to add event from suggestion');
    } finally {
      setSuggestionLoading(false);
    }
  };
        ...
        <div className="mb-6 bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-2xl p-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              {eventSuggestion.status !== 'IMPLEMENTED' && (
                <button
                  onClick={() => implementSuggestion(eventSuggestion.id)}
                  disabled={suggestionLoading}
                  className="flex items-center bg-[#FF4500] text-white px-4 py-2 rounded-xl hover:bg-[#E03E00] transition-colors text-sm font-semibold disabled:opacity-50"
                >
                  <PlusIcon className="h-4 w-4 mr-1" />
                  {suggestionLoading ? 'Adding...' : 'Add to Events'}
                </button>
              )}
            </div>
          </div>
