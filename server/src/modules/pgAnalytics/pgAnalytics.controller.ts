...
export const pgCommunityAnalyticsController = {
  ...
  async createPgCommunityEvent(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      if (!req.user) {
        throw new AppError('Authentication required', 401);
      }
      ...
      const { id } = req.params;
      const event = await pgCommunityAnalyticsService.createPgCommunityEvent(
        id,
        req.user.userId,
        req.user.role,
        req.body
