...
/**
 * @route POST /api/pg-analytics/:id/events
 * @desc Create a new event for a PG community
 * @access Private (PG Owner only)
 */
router.post(
  '/:id/events',
  authenticateToken,
  validate(analyticsValidationSchemas.createPgEventSchema),
  pgCommunityAnalyticsController.createPgCommunityEvent
);
